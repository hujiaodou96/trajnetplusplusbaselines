...
from .modules import Hidden2Normal, InputEmbedding, Pred_Goal
...
class LSTM(torch.nn.Module):
    def __init__(self, embedding_dim=64, hidden_dim=128, pool=None, pool_to_input=True, goal_dim=None, goal_flag=False):
        ...
        # Predict the goal coordinates x, y of main pedestrian of each track
        self.pred_goal = Pred_Goal(self.hidden_dim)

    def step(self, lstm, hidden_cell_state, obs1, obs2, batch_split, flag, goals=None):
        ...
        flag: String ["pred_goals" or "pred_outputs"]
            Indicate which outputs should return
        ...
        if "pred_goals", return only hidden_cell_state; else return both
        """
        ...
        # if use the goal info to predict outputs
        if self.goal_flag and flag == "pred_outputs":
        ...
        # if predict the trajectories, return hidden_cell_state and normal for the prediction of each future time step
        if flag == "pred_outputs":
        ...
        # if predict the goals, only return hidden_cell_stacked as each track only has one output coordinates
        elif flag == "pred_goals":
            return hidden_cell_stacked

    def forward(self, observed, batch_split, flag, goals=None, prediction_truth=None, n_predict=None):
        ...
        if predict goals:
            pred_goal_scenes: Tensor [num_tracks, 2] ?
                Predicted goal positions of pedestrians
        ...
        # predict the goal coordinates
        if flag == "pred_goals":
            # input the trajectories of each time step into lstm
            for obs1, obs2 in zip(observed[:-1], observed[1:]):
                hidden_cell_state = self.step(self.encoder, hidden_cell_state, obs1, obs2,
                                              batch_split, flag="pred_goals")
            # use the last hidden state to produce the predicted goals coordinates
            pred_goals = self.pred_goal(hidden_cell_state[0])
            # pred_goal: Tensor [num_tracks, 2]
            #    Predicted goal positions of pedestrians
            pred_goals = pred_goals.detach()
            return pred_goals


        # predict the trajectories at future time steps
        elif flag == "pred_outputs":
            # list of predictions
...
class LSTMPredictor(object):
    ...
    def __call__(self, paths, scene_goal, n_predict=12, modes=1, predict_all=True, obs_length=9, start_length=0,
                 args=None):
        self.model.eval()
        # self.model.train()
        with torch.no_grad():
